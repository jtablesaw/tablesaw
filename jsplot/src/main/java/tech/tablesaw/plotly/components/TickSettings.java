...
public class TickSettings {
    ...
    private final int length;      // (number greater than or equal to 0) default: 5 Sets the tick length (in px).
    private final int width;       //(number greater than or equal to 0) default: 1 , Sets the tick width (in px).
    private final String color;    //(color) default: "#444" Sets the tick color.
    private final boolean showLabels; //(boolean) default: True Determines whether or not the tick labels are drawn.
    ...
    public static class TickSettingsBuilder {
         ...
         * @param tickText      Sets the text displayed at the ticks position via `tickvals`.
         ...
         * If "last", only the last tick is displayed with a prefix. If "none", tick prefixes are hidden.
         */
         ...
         *
         * If the axis `type` is "category", it should be a number, using the scale where each category
