 ...
 *
 */
public class Scatter implements GlimpseLayoutProvider {
  ...
  public Scatter(String plotTitle, String yAxisName, NumericColumn x, NumericColumn ... seriesYvalues)
      throws IOException {
  ...
  public Scatter(String plotTitle, NumericColumn x, NumericColumn y)
      throws IOException {
    this.plotTitle = plotTitle;
    this.x = x;
    this.xAxisName = x.name();
  ...
  public Scatter(String plotTitle, String xAxisName, String yAxisName, NumericColumn x, NumericColumn ... seriesYvalues)
      throws IOException {
    this.plotTitle = plotTitle;
    this.x = x;
  ...
  public Scatter(NumericColumn x, NumericColumn ... seriesYvalues) throws IOException {
  ...
  public static void show(NumericColumn x, NumericColumn ... seriesYvalues) {
    ...
    try {
      Display.showWithSwing(new Scatter(x, seriesYvalues));
    } catch (Exception e) {
      e.printStackTrace();
  ...
  public static void show(String title, NumericColumn x, NumericColumn y) {
    ...
    try {
      Display.showWithSwing(new Scatter(title, x, y));
    } catch (Exception e) {
      e.printStackTrace();
      throw new RuntimeException(e);
    }
  ...
  public GlimpseLayout getLayout() {
    ...
    List<Color> colors = StandardColors.randomColors(seriesYvalues.length);
    ...
    for (int col = 0; col < seriesYvalues.length; col++) {
      ...
      for (int i = 0; i < seriesYvalues[col].size(); i++) {
        pointAccumulator.add(i, x.getFloat(i), seriesYvalues[col].getFloat(i), GlimpseColor.fromColorAwt(colors.get(col)));
      }
  ...
  static class SimpleScatter implements GlimpseLayoutProvider {
    ...
    public GlimpseLayout getLayout() {
      ...
      List<StandardColor> colors = StandardColors.getFilteredColors(Hue.Red_10, Range.atLeast(4), Range.atLeast(4));
      double yMin = StatUtils.min(yData);
      double yMax = StatUtils.max(yData);

      for (int i = 0; i < xData.length; i++) {
        pointAccumulator.add(i, (float) xData[i], (float) yData[i], GlimpseColor.fromColorAwt(colors.get(0).asColor()));
      }
      painter.putPoints(pointAccumulator);
  ...
  static class GroupScatter implements GlimpseLayoutProvider {
    ...
    public GlimpseLayout getLayout() {
      ...
      List<Color> colors = StandardColors.randomColors(series.size());
      double yMin = Double.MAX_VALUE;
      double yMax = 0;
      ...
      for (int viewNo = 0; viewNo < series.size(); viewNo++) {
        ...
        for (int i = 0; i < view.rowCount(); i++) {
          pointAccumulator.add(i, xCol.getFloat(i), yCol.getFloat(i), GlimpseColor.fromColorAwt(colors.get(viewNo)));
        }
        painter.putPoints(pointAccumulator);
      }
