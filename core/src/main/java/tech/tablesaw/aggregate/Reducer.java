...
public class Reducer {
     ...
     * by applying the given functions
     */
    public Reducer(Table sourceTable, Column numericColumn1, Column numericColumn2, Reduction... functions) {
        this.original = sourceTable;
     ...
     * by applying the given functions
     */
    public Reducer(Table sourceTable,
    ...
    public Table apply() {
        ...
        for (String columnName : summarizedColumns) {
            for (Reduction function : reductions) {
    ...
    private Table summarize(TableSliceGroup group) {
        ...
        ArrayListMultimap<String, Reduction> reductionMultimap = ArrayListMultimap.create();
        ...
        for (String name : reductionMultimap.keys()) {
            List<Reduction> reductions = reductionMultimap.get(name);
            results.add(group.aggregate(name, reductions.toArray(new Reduction[reductions.size()])));
        }
