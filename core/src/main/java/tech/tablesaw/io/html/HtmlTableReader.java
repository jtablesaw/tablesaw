...
public class HtmlTableReader {
	...
	public String tableToCsv(Elements tables) throws IOException {
		if (tables.size() != 1) {
	...
	public String tableToCsv(Document doc) throws IOException {
		return tableToCsv(doc.select("table"));
	}

	public String tableToCsv(String url) throws IOException {
		return tableToCsv(Jsoup.connect(url).get());
	}
