...
public class OrcWriter implements DataWriter<OrcWriteOptions> {
  ...
  public void write(Table table, OrcWriteOptions options) throws IOException {
    ...
    if (options.getWriterOptions()
        != null) { // Executed if TypeDescription and writer option provide by user
      ...
      for (Column<?> column : columns) {
        if (!sawOrcTypeMapping.containsKey(column.type()))
          throw new IllegalArgumentException(
              "Unable to find relevant ORC data type to store: " + column.type());
        typeDescription.addField(column.name(), sawOrcTypeMapping.get(column.type()));
      }
  ...
  interface ITypeConverter {
    ...
    void convertToOrcType(
        Column<?> column, int rowIndexInJTable, ColumnVector columnVector, int row);

    default boolean checkAndHandleNull(
        Column<?> column, int rowIndexInJTable, ColumnVector columnVector, int row) {
