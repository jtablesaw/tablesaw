...
public class OrcReader implements DataReader<OrcReadOptions> {
  ...
  public Table read(OrcReadOptions options) throws IOException {
    ...
    try (Reader reader =
        OrcFile.createReader(new Path(options.source().file().getAbsolutePath()), readerOptions)) {
  ...
  private Table readFromOrcFile(Reader reader, OrcReadOptions options) throws IOException {
    ...
    List<String> columnTypes =
        reader.getSchema().getChildren().stream()
  ...
  private List<String> getUnsupportedTypes(List<String> columnTypes) {
    return columnTypes.stream()
        .filter(str -> !supportedTypeMap.containsKey(str))
        .collect(Collectors.toList());
  }

  private void appendValue(
      final Column<?> column,
      final VectorizedRowBatch readBatch,
      final int rowIndex,
    ...
    if (!readBatch.cols[columnIndex].isNull[currentRowIndex]
        || readBatch.cols[columnIndex].noNulls) {
      switch (type) {
        case "boolean":
        ...
        case "date":
          ...
          break;
        case "decimal":
          // Decimal can hold values of many types such as float, int, byte, short etc for
          // simplicity
          // we will store values in double
          DecimalColumnVector decimalColumnVector =
              (DecimalColumnVector) readBatch.cols[columnIndex];
        ...
        default:
          ...
          String str =
              new String(
                  strCol.vector[currentRowIndex],
                  strCol.start[currentRowIndex],
                  strCol.length[currentRowIndex]);
