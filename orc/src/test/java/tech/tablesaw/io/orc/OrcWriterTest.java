...
public class OrcWriterTest {
  ...
  public void testWritePositive() {
    ...
    StringColumn stringColumn =
        StringColumn.create("name", Arrays.asList("john", "marry", "cassey", "batman"));
    TextColumn textColumn =
        TextColumn.create(
            ...
            Arrays.asList(
                "12th downing street",
                "342th Sideway road",
                "90th Riverfornt apartment",
                "Wane Manor, Gotham"));
    BooleanColumn booleanColumn =
        BooleanColumn.create("married", Arrays.asList(false, true, false, true));
    IntColumn intColumn = IntColumn.create("age", new int[] {21, 34, 12, 35});
    LongColumn longColumn =
        LongColumn.create("phone", new long[] {9420132578L, 9420132978L, 9454132578L, 9465132578L});
    ShortColumn shortColumn = ShortColumn.create("id", new short[] {102, 108, 765, 245});
    DoubleColumn doubleColumn =
        DoubleColumn.create(
            "balance",
            new double[] {89523600.987, 95222111000.9008, 78855522200.97, 9600000000000.9000});
    ...
    InstantColumn instantColumn =
        InstantColumn.create(
            "instant", Arrays.asList(Instant.now(), Instant.now(), Instant.now(), Instant.now()));
    DateColumn dateColumn =
        DateColumn.create(
            "DOB",
            Arrays.asList(
                LocalDate.of(1982, 2, 21),
  ...
  public void testWriteWithSomeNulls() {
    ...
    StringColumn stringColumn =
        StringColumn.create("name", Arrays.asList("john", null, "cassey", "batman"));
    TextColumn textColumn =
        TextColumn.create(
            "address",
            Arrays.asList(
                "12th downing street", "342th Sideway road", "90th Riverfornt apartment", null));
    BooleanColumn booleanColumn =
        BooleanColumn.create("married", Arrays.asList(false, true, false, null));
    IntColumn intColumn = IntColumn.create("age", new Integer[] {21, 34, null, 35});
    LongColumn longColumn =
        LongColumn.create(
            "phone", LongStream.of(9420132578L, 9420132978L, 9454132578L, 9465132578L));
    ShortColumn shortColumn = ShortColumn.create("id", new Short[] {102, 108, 765, 78});
    DoubleColumn doubleColumn =
        DoubleColumn.create(
            "balance", new Double[] {89523600.987, 95222111000.9008, 78855522200.898, null});
    FloatColumn floatColumn = FloatColumn.create("weight", new Float[] {98.23f, null, 25f, null});
    InstantColumn instantColumn =
        InstantColumn.create(
            "instant", Arrays.asList(Instant.now(), Instant.now(), null, null)); // Appa
    DateColumn dateColumn =
        DateColumn.create(
            "DOB",
            Arrays.asList(
    ...
    DateTimeColumn timeColumn =
        DateTimeColumn.create(
            "time",
            Arrays.asList(null, LocalDateTime.now(), LocalDateTime.now(), LocalDateTime.now()));
    table.addColumns(
        stringColumn,
        textColumn,
        booleanColumn,
