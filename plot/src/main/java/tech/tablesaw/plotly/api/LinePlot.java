...
public class LinePlot {
    ...
    for (int i = 0; i < tables.size(); i++) {
      ...
      traces[i] =
          ScatterTrace.builder(
                  tableList.get(i).numberColumn(xCol).asDoubleArray(), tableList.get(i).numberColumn(yCol).asDoubleArray())
              .showLegend(true)
  ...
  public static Figure create(String title, Table table, String xCol, String yCol) {
    ...
    ScatterTrace trace =
        ScatterTrace.builder(table.numberColumn(xCol).asDoubleArray(), table.numberColumn(yCol).asDoubleArray())
            .mode(ScatterTrace.Mode.LINE)
            .build();
